...
$(BUILD)/layout-top-outlineonly.eps: $(PROJECT).pcb | $(BUILD)
	pcb -x eps --layer-stack "outline" --layer-color-3 '#FF8800' --as-shown --eps-file $@ $^
...
$(BUILD)/layout-bottom-outlineonly.eps: $(PROJECT).pcb | $(BUILD)
	pcb -x eps --layer-stack "outline,solderside" --layer-color-3 '#FF8800' --as-shown --eps-file $@ $^
